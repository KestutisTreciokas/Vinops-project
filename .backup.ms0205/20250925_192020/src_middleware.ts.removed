...
// Lightweight VIN validation per SSOT
...
export async function middleware(req: NextRequest) {
  ...
  if (parts.length >= 3 && parts[1] === 'vin') {
    ...
    // 2) Suppress (410) / Invalid (422->404) by probing API
    try {
      ...
      // fail-open to page SSR; page will still handle invalid via notFound()
    }
    return NextResponse.next();
  }

  // Cars route: add X-Robots-Tag for complex filters
